...
public class Kernel {
  ...
  public static final String[] sys_errlist = {
      null, "Not owner", "No such file or directory", null, null, null, null, null, null, "Bad file number", null, null,
      null, "Permission denied", null, null, null, "File exists", "Cross-device link", null, "Not a directory",
      "Is a directory", "Invalid argument", "File table overflow", "Too many open files", null, null, "File too large",
      "No space left on device", null, "Read-only file system", "Too many links"
  };
  ...
  /*Se usa para definir una mascara de bts, esto ayuara para determinar 
   * el tipo de archivo.
   * (short) 0170000; se usa como mascara, esta en notaion octal
   * los primeros 4 bist audaran representar el tipo de archivo 
   */
  ...
  /*Representa directorio, se esta representando en noatcion octal  */
   ...
   * @throws Exception
   */
  /*LO que se busca es verificar si es un directorio o un archivo
   * isDirectory toma a pathname de igual forma lanza una excepcion en caso de encontrar error
   * se llama a findIndexNode donde pasa pathname y indexNode, esto busca un indie el cual este relacionado
   * se verifica que indexNodeNumber es menos q 0, si lo es se imprime el tedxto
   * si es >= (indice valido ), verifica que el modo de index tiene establecido 
   * bir S_IFDIR el cual representa directorio, y se imprime que es un directorio
   * Y finalmente si no es igual a S_IFDIR quiere decir que es un archivo
   */
  public static void isDirectory(String pathname) throws Exception {
    IndexNode indexNode = new IndexNode();
    short indexNodeNumber = findIndexNode(pathname, indexNode);

    if (indexNodeNumber < 0) {
      System.err.println("cp: El archivo o directorio no existe");       
    } else {
      if ((indexNode.getMode() & S_IFMT) == S_IFDIR) {
        System.err.println("cp: Es un  directorio");
      } else {
        System.err.println("cp: Es un  archivo");
      }
    }
    //Kernel.exit( 1 ) ;
  }
